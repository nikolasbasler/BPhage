...
phage_tpm_meta_merge.Hives <- read.csv("output/R/relative_abundance/phage_tpm_meta_merge.Hives.csv")
...
sulf_meta <- phage_tpm_meta_merge.Hives %>% tibble() %>%
  pivot_longer(-contig, names_to = "Hive_ID", values_to = "TPM") %>%
  mutate(Hive_ID = as.integer(str_split_i(Hive_ID, "_", 2))) %>%
  ...
  select(contig, Hive_ID, TPM, sulf_metabolism, Country, Family, Host_group, Core) %>%
  # filter(Gut_part == "rec") %>% # Focus on rectum for more stable values (some midgut samples have very few phages)
  group_by(Hive_ID, sulf_metabolism) %>% 
  mutate(sulf_tpm = sum(TPM)) %>%
  mutate(Hive_ID = factor(Hive_ID)) %>%
  ungroup()

tpm_tibble <- sulf_meta %>%
  filter(sulf_metabolism) %>%
  select(Hive_ID, Country, sulf_tpm) %>%
  distinct()
genomes_tibble <- sulf_meta %>%
  filter(TPM > 0) %>%
  group_by(Hive_ID, sulf_metabolism) %>% 
  mutate(sulf_genomes = n()) %>%
  group_by(Hive_ID) %>%
  mutate(sulf_genome_props = sulf_genomes / n()) %>%
  ungroup() %>%
  filter(sulf_metabolism) %>% 
  select(Hive_ID, Country, sulf_genomes, sulf_genome_props) %>%
  distinct() %>%
  arrange(Country)
sulf_stats <- left_join(tpm_tibble, genomes_tibble, by = c("Hive_ID", "Country")) %>% 
  mutate(sulf_genomes = ifelse(sulf_tpm == 0 , 0, sulf_genomes),
         sulf_genome_props = ifelse(sulf_tpm == 0 , 0, sulf_genome_props))


hive_kw <- list()
hive_plot <- list()
for (thing in c("sulf_tpm", "sulf_genome_props")) {
  hive_kw[[thing]] <- kruskal.test(sulf_stats[[thing]] ~ sulf_stats$Country)
  hive_plot[[thing]] <- sulf_stats %>%
    ggplot(aes(x = Country, y = .data[[thing]])) +
    geom_boxplot(outliers = FALSE) +
    geom_jitter(alpha = 0.33, width = 0.25, height = NULL) +
    ggtitle(paste0("Hive ", thing, " - KW p-value: ", hive_kw[[thing]]$p.value))
}


# left_join(tpm_tibble, cropland_fraction, by = "Country") %>%
#   ggplot(aes(x = cropland_fraction_2k_radius, y = sulf_tpm)) +
#   geom_point() +
...
for (element in elements) {
  for (item in c("Pesticides (total)", "Insecticides", "Herbicides", "Fungicides and Bactericides")) {
    pest_cor_tibbles[[element]][[item]] <- FAOSTAT_added_data %>%
      select(all_of(c("Country", "Item", element))) %>%
      filter(Item == item) %>%
      left_join(., sulf_stats, by = "Country")
    
    pest_cor_plots[[element]][[item]] <- pest_cor_tibbles[[element]][[item]] %>%
      ggplot(aes(x = .data[[element]], y = sulf_tpm)) +
      geom_point() +
      geom_smooth(method = "glm", formula = y ~ x) +
      stat_cor(method = "spearman") +
      ggtitle(paste0(element, " - ", item))
