...
for (hoi in hosts_of_interest) {
  ...
  model_tpm_simple_cropland[[hoi]] <- lmer(log_tpm ~ Cropland_in_2km_radius + Gut_part + Season +
...
for (hoi in hosts_of_interest) {
  ...
  model_tpm_simple_total_pest[[hoi]][["Pesticides (total)"]] <- lmer(log_tpm ~ est_use_in_2k_radius + Gut_part + Season +
...
for (hoi in hosts_of_interest) {
  for (item in c("Insecticides", "Herbicides", "Fungicides and Bactericides", "Plant Growth Regulators")) {
    ...
    model_tpm_simple_pest_groups[[hoi]][[item]] <- lmer(log_tpm ~ est_use_in_2k_radius + Gut_part + Season +
                                                           (1 | Hive_ID ), data = temp_test_tibble)
...
for (hoi in hosts_of_interest) {
  for (item in spec_pests) {
    temp_test_tibble <- test_tibble_log_tpm[[hoi]] %>% 
      rename(est_use_in_2k_radius = all_of(item))
    model_tpm_simple_specific_pests[[hoi]][[item]] <- lmer(log_tpm ~ est_use_in_2k_radius + Gut_part + Season +
                                                              (1 | Hive_ID ), data = temp_test_tibble)
...
plot(model_tpm_simple_specific_pests$Bifidobacterium$`Herbicides – Dinitroanilines`)
qqnorm(resid(model_tpm_simple_specific_pests$Bifidobacterium$`Herbicides – Dinitroanilines`))
