...
test_tibble_npat <- pathogen_ct %>%
  group_by(Bee_pool) %>%
  count(detection = Ct < 41, name = "n_pathogens") %>%
  ungroup() %>%
  filter(detection) %>%
  select(-detection) %>%
  left_join(., metadata[c("Bee_pool", "Country", "Hive_ID", "Season")], by = "Bee_pool") %>%
...
model_npat_simple_cropland <- list()
coeffs_npat_simple$cropland <- tibble()

model_npat_simple_cropland <- lmer(n_pathogens ~ ha_cropland_in_2k_radius + Season +
       (1 | Hive_ID ), data = test_tibble_npat)
summary(model_npat_simple_cropland)

burden <- pathogen_ct %>%
  filter(Ct < 41) %>%
  mutate(burden = 2^-Ct) %>%
  group_by(Bee_pool) %>%
  mutate(total_burden = sum(burden)) %>%
  ungroup() %>%
  select(Bee_pool, total_burden) %>%
  distinct() %>%
  left_join(., metadata[c("Bee_pool", "Country", "Hive_ID", "Season")], by = "Bee_pool") %>%
  distinct() %>%
  left_join(., cropland_and_FAO, by = "Country")

model_burden_simple_cropland <- lmer(total_burden ~ ha_cropland_in_2k_radius + Season +
                                     (1 | Hive_ID ), data = burden)
summary(model_burden_simple_cropland)
