...
vibrant_features <- left_join(VIBRANT_annotations, VIBRANT_annotation_coords, by = join_by(protein == CDS)) %>%
  ...
  select(seq_id, length, genome_label, start, end, strand, phrog, funct, 
         product, type, fill_group, gene_label, scaffold, 
         distance_of_goi_to_contig_edge)
...
remove_for_stringency$`PAPS reductase` <- tibble(
  contig = c(
    ...
    "NODE_A8_length_37910_cov_326.687099_PT_19414_sum_rec_d" # redundant: also transferred from pharokka
    
    # "NODE_A2_length_44969_cov_51.188675_PT_19409_aut_mid_d", # redundant: also transferred from pharokka
    # "NODE_A4_length_44865_cov_65.538894_PT_19412_spr_rec_d",
    # 
    # "NODE_A2_length_44969_cov_51.188675_PT_19409_aut_mid_d", # redundant: also high evalue
    # "NODE_A8_length_37910_cov_326.687099_PT_19414_sum_rec_d" # redundant: also at edge of contig
  ),
  reason = c(
...
classification %>%
  filter(contig %in% unique(paps_coords$seq_id),
