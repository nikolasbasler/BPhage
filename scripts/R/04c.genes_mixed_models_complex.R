...
for (goi in "phosphoadenosine phosphosulfate reductase") {
  ...
  summary(model_tpm_complex_cropland[[goi]])
...
for (goi in "phosphoadenosine phosphosulfate reductase") {
  ...
  model_tpm_complex_total_pest[[goi]][["Pesticides (total)"]] <- lmer(log_tpm ~ est_use_in_2k_radius + Gut_part + Season +
                                                                        (1 | Country / Hive_ID ), data = temp_test_tibble)
  
  coeffs_tpm_complex$total_pest <- summary(model_tpm_complex_total_pest[[goi]][["Pesticides (total)"]])$coefficients %>%
    as.data.frame() %>%
    rownames_to_column("metric") %>%
    tibble() %>%
    mutate(gene = goi, 
           ...
           .before = metric) %>%
    mutate(singular = ifelse(isSingular(model_tpm_complex_total_pest[[goi]][["Pesticides (total)"]]), TRUE, FALSE)) %>%
...
for (goi in "phosphoadenosine phosphosulfate reductase") {
  for (item in c("Insecticides", "Herbicides", "Fungicides and Bactericides", "Plant Growth Regulators")) {
    ...
    model_tpm_complex_pest_groups[[goi]][[item]] <- lmer(log_tpm ~ est_use_in_2k_radius + Gut_part + Season +
                                                           (1 | Country / Hive_ID ), data = temp_test_tibble)
    
    has_convergence_issues <- FALSE
    messages <- model_tpm_complex_pest_groups[[goi]][[item]]@optinfo$conv$lme4$messages
    ...
    if (has_convergence_issues) {
      ...
      coeffs_tpm_complex$pest_groups <- summary(model_tpm_complex_pest_groups[[goi]][[item]])$coefficients %>%
        as.data.frame() %>%
        rownames_to_column("metric") %>%
        tibble() %>%
        mutate(gene = goi, 
               Item = item,
               .before = metric) %>%
        mutate(singular = ifelse(isSingular(model_tpm_complex_pest_groups[[goi]][[item]]), TRUE, FALSE)) %>%
...
for (goi in "phosphoadenosine phosphosulfate reductase") {
  for (item in spec_pests) {
    temp_test_tibble <- test_tibble_log_tpm[[goi]] %>% 
      rename(est_use_in_2k_radius = all_of(item))
    model_tpm_complex_specific_pests[[goi]][[item]] <- lmer(log_tpm ~ est_use_in_2k_radius + Gut_part + Season +
                                                              (1 | Country / Hive_ID ), data = temp_test_tibble)
    
    has_convergence_issues <- FALSE
    messages <- model_tpm_complex_specific_pests[[goi]][[item]]@optinfo$conv$lme4$messages
    if (is.null(messages)) {
      has_convergence_issues <- FALSE
    } else {
    ...
    if (has_convergence_issues) {
      print(paste0(goi, "; ", item, " - Model didn't converge. Removed from the list."))
      model_tpm_complex_specific_pests[[goi]][[item]] <- NULL
      
    } else {
      coeffs_tpm_complex$specific_pests <- summary(model_tpm_complex_specific_pests[[goi]][[item]])$coefficients %>%
        as.data.frame() %>%
        rownames_to_column("metric") %>%
        tibble() %>%
        mutate(gene = goi, 
               Item = item,
               .before = metric) %>%
        mutate(singular = ifelse(isSingular(model_tpm_complex_specific_pests[[goi]][[item]]), TRUE, FALSE)) %>%
